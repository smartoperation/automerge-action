...
async function executeGitHubAction(context, eventName, eventData) {
  ...
  if (eventName === "push") {
    ...
    await handlePullRequestCheck(context, eventName, eventData);
  } else {
...
async function handlePullRequestCheck(context, eventName, event) {
  const { action } = event;
  ...
  if (event.check_suite.status !== "completed") {
    logger.info("Check suite is not completed:", event.check_suite.status);
    logger.info("Action ignored:", eventName, action);
    return;
  }

  let updated = 0;
  for (const pullRequest of event.check_suite.pull_requests) {
    try {
